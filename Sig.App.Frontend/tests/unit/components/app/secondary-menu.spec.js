...
describe("secondary-menu.vue", () => {
  ...
  // CRCL-2684 : le rapport d'enveloppe n'existe qu'au niveau du programme. L'offrir aux gestionnaires
  // de groupe de participant·e·s les menait sur une page vide.
  it("offers only the subscription report to an organization manager", () => {
    const wrapper = mountMenu({ [CLAIM_UTYPE]: CLAIM_UTYPE_ORGANIZATIONMANAGER }, organizationManagerPermissions);

    expect(reportLinks(wrapper)).toContain(URL_SUBSCRIPTION_END_REPORT);
    expect(reportLinks(wrapper)).not.toContain(URL_BUDGET_ALLOWANCE_REPORT);
  });
});
