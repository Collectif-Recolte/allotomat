<template>
  <div class="pf-table flex flex-col relative">
    <div ref="containerRef" :style="isOverflowing ? { transform: 'translate3d(0,0,0)' } : {}" :class="[
      'pf-table__overflowing-content -mx-section md:-mx-8 max-h-simple-table-height',
      ...
      <div class="pb-2 align-middle inline-block min-w-full px-section md:px-8">
        <div class="" :class="{ 'pb-12': slots.floatingActions || hasBottomPadding }">
          <table ref="tableRef" class="min-w-full divide-y-2 divide-primary-900 dark:divide-grey-700">
            <slot name="caption">
            ...
            <thead class="sticky z-10 top-0 bg-grey-50">
              <tr
                ...
                <th v-for="(col, index) in props.cols" :key="index" scope="col"
                  ...
                  <div
                    class="hidden group-one-first:block bg-grey-50 absolute inset-y-0 -left-section right-full md:-left-8">
                  </div>
                  {{ col.label }}
                </th>
              </tr>
            ...
            <tfoot v-if="props.footers.length > 0" class="sticky z-10 bottom-0 bg-grey-50">
              <tr
                class="relative after:absolute after:left-full after:-right-section md:after:-right-8 after:inset-y-0 after:bg-grey-50">
                <th v-for="(footer, index) in props.footers" :key="index" scope="col"
                  class="px-6 py-4 group-one first:pl-0 last:pr-0 text-p2 font-semibold text-primary-900 dark:text-grey-300 relative"
    ...
    <div v-if="slots.floatingActions"
...
<script setup>
import { onMounted, onUnmounted, ref, useSlots, defineProps } from "vue";
...
onMounted(() => {
  resizeObserver = new ResizeObserver(() => {
    if (containerRef.value) {
      ...
      if (isOverflowing.value !== nowOverflowing) {
