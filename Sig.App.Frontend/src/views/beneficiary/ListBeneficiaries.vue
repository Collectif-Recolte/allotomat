  ...
  <Loading :loading="loading || projectsLoading || organizationsLoading || loadingAllBeneficiaries" is-full-height>
    <RouterView v-slot="{ Component }">
      <Title :title="t('title')" :subpages="subpages">
        <template
          v-if="organizations && organizations.length > 0 && !administrationSubscriptionsOffPlatform && !isAllGroupSelected"
          #center>
        ...
        <template v-if="organizations && organizations.length > 0 && manageOrganizations" #right>
...
watch(resultOrganizations, (value) => {
  if (value === undefined || value.organizations.length === 0) return;
