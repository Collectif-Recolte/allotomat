        ...
        public override async Task<ILookup<string, MarketGroupGraphType>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            switch (manager.Type)
            {
                case UserType.ProjectManager:
                {
                    ...
                    var results = await db.MarketGroups.Where(x => !x.IsArchived && existingMarketClaims.Contains(x.ProjectId.ToString())).ToListAsync();
                ...
                case UserType.MarketGroupManager:
                {
                    ...
                    var results = await db.MarketGroups.Where(x => !x.IsArchived && existingMarketGroupClaims.Contains(x.Id.ToString())).ToListAsync();
